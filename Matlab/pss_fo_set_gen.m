...
for i=1:num_pss
    sp = (i-1)*num_fo + 1; % Index Start
    ep = sp + num_fo - 1;  % Index End
    % Perform IDFT On All The Possible PSS Sequences and Frequency Offsets
    pss_fo_set(:,sp:ep) = repmat(pss(:,i),1,num_fo).*exp(1i.*2.*pi.*(1./sampling_rate).*(0:(len_pss-1)).'*fo_search_set);
    % Complex Conjugate? And Normalization
